...
class SelectAddressScreen extends StatelessWidget {
                            ...
                            () => Container(
                              ...
                              child: Stack(
                                children: [
                                    ...
                                    children: [
                                      RadioGroup<int>(
                                        ...
                                        onChanged: (value) async {
                                          if (value != null) {
                                            final selectedAddress = selectAddressController.addressList.firstWhere((e) => e.id.value == value);

                                            if (!isFromProfile) {
                                              /// LogisticZone Api
                                              await selectAddressController.getLogisticZoneApi(addressId: selectedAddress.id.value);
                                              selectAddressController.isSelectedLogistic(false);
                                              selectAddressController.setLogisticZoneData(LogisticZoneData());
                                            }

                                            selectAddressController.setSelectedAddressData(selectedAddress);
                                          }
                                        },
                                        child: RadioListTile<int>(
                                          value: addressData.id.value,
                                          activeColor: primaryColor,
                                          shape: RoundedRectangleBorder(borderRadius: radius()),
                                          visualDensity: VisualDensity.compact,
                                          dense: true,
                                          contentPadding: const EdgeInsets.all(8),
                                          title: "${addressData.firstName} ${addressData.lastName}".trim().isEmpty
                                              ? null
                                              : Text(
                                  ...
                                  Positioned(
                                    right: selectedLanguageCode.value != 'ar' ? 0 : null,
                                    left: selectedLanguageCode.value == 'ar' ? 0 : null,
                                    child: Container(
                                      padding: const EdgeInsets.symmetric(vertical: 4, horizontal: 16),
                                      decoration: boxDecorationWithRoundedCorners(
                                        backgroundColor: lightPrimaryColor,
                                        borderRadius: radiusOnly(
                                          bottomLeft: selectedLanguageCode.value != 'ar' ? defaultRadius : null,
                                          bottomRight: selectedLanguageCode.value == 'ar' ? defaultRadius : null,
                                        ),
                                      ),
              ...
              children: [
                Obx(
                  () => AppButton(
                    ...
                    textStyle: selectAddressController.isSelectedLogistic.value ? appButtonTextStyleGray : appButtonTextStyleWhite,
                    color: selectAddressController.isSelectedLogistic.value ? lightSecondaryColor : primaryColor,
                    onTap: () {
  ...
  Widget buildIconWidget({required String icon, required VoidCallback onTap, Color? color}) {
    return SizedBox(
      height: 38,
      width: 38,
      child: IconButton(padding: EdgeInsets.zero, icon: icon.iconImage(size: 18, color: color ?? primaryColor), onPressed: onTap),
    );
  }
            ...
            return Obx(
              () => Container(
                ...
                child: RadioGroup<int>(
                  ...
                  onChanged: (value) {
                    if (value != null) {
                      final selectedLogistic = selectAddressController.logisticList.firstWhere((e) => e.id == value);

                      selectAddressController.setLogisticZoneData(selectedLogistic);
                  ...
                  child: RadioListTile<int>(
                    ...
                    title: Text(
                      logisticData.name,
                      style: primaryTextStyle(
                        color: isDarkMode.value ? textPrimaryColorGlobal : secondaryColor,
                        size: 14,
                      ),
                    ),
                    secondary: Row(

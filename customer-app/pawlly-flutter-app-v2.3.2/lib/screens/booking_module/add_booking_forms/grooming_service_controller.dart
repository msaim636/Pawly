...
class GroomingController extends GetxController {
  ...
  void getService({String searchtext = ""}) {
    ...
    PetServiceFormApis.getService(
      ...
      if (isUpdateForm.value) {
        ...
        selectedService(value.firstWhere((p0) => p0.id == bookingFormData.value.veterinaryServiceId, orElse: () => ServiceModel()));
        ...
        currentSelectedService.value.serviceAmount = selectedService.value.defaultPrice.toDouble();
        showBookBtn(false);
        showBookBtn(true);
        getGroomer();
      }
  ...
  ///Get Groomer List
  void getGroomer({String searchtext = ""}) {
    isLoading(true);

...
class TrainingController extends GetxController {
    ...
    if (Get.arguments is BookingDataModel) {
      try {
        ...
        duration(PetServiceFormApis.getTrainingDuration(
          ...
          if (selectedDuration.value.id > 0 && selectedDuration.value.amount > 0) {
            currentSelectedService.value.serviceAmount = selectedDuration.value.amount.toDouble();
  ...
  void getTraining({int? employeeId, int? isBooking, String searchtext = "", bool isFromSwipeRefresh = false}) {
  ...
  ///Get Trainner List
  void getTrainer({String searchtext = ""}) {
    isLoading(true);
  ...
  void onTrainerSearchChange(String searchtext) {
    searchTrainerFunc(
      searchtext: searchtext,

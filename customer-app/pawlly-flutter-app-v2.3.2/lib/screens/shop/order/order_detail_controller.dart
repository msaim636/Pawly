...
class OrderDetailController extends GetxController {
  ...
  RxBool isShowInvoiceOption = false.obs;
  RxInt orderId = 0.obs;
  ...
  void submit(bool addReview) async {
    ...
    await OrderApis.updateOrderReview(
      ...
      onSuccess: (data) {
        ...
        toast(addReview ? locale.value.reviewAddedSuccessfully : locale.value.reviewUpdatedSuccessfully);
      },
    ).catchError((e) {
      isLoading(false);
      toast(e.toString());
  ...
  Future<void> getInvoice(int orderId) async {
    ...
    await OrderApis.downloadInvoicePdf(orderId: orderId)
        .then(
          (value) {
            if (value.isNotEmpty) {
  ...
  void mergePickedFiles(RxList<XFile> otherList) {
    if (otherList.isNotEmpty) {
      pickedFile.addAll(otherList);
    }
  }
}

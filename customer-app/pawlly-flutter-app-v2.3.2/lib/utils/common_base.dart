...
Widget detailWidget({required String title, required String value, Color? textColor, TextStyle? textStyle}) {
  return Row(
    ...
    children: [
      ...
      Text(value, textAlign: selectedLanguageCode.value == 'ar' ? TextAlign.left : TextAlign.right, style: textStyle ?? primaryTextStyle(size: 12, color: textColor)).expand(),
    ],
  ).paddingBottom(10).visible(value.isNotEmpty);
}

Widget detailWidgetPrice({required String title, required num value, Color? textColor, bool isSemiBoldText = false, bool isInfoIconShow = false, void Function()? onTap}) {
  return Row(
    mainAxisAlignment: MainAxisAlignment.spaceBetween,
    children: [
      ...
      Row(
        children: [
          if (isInfoIconShow)...[
            GestureDetector(
              onTap: onTap,
              child: Image.asset(Assets.profileIconsIcInfoOutlined, width: 20, height: 20),
            ),
            8.width
          ],
          PriceWidget(

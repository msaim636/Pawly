...
class ShopProductAPI {
  ...
  static Future<RxList<ShopCategory>> getShopCategory({
    ...
    final categoryRes =
        CategoryListResponse.fromJson(await handleResponse(await buildHttpResponse('${APIEndPoints.getShopCategory}$pageQuery$perPageQuery$searchCategory', method: HttpMethodType.GET)));
  ...
  static Future<RxList<ProductItemDataResponse>> getProduct({
    ...
    final productRes = ProductListResponse.fromJson(
        await handleResponse(await buildHttpResponse('${APIEndPoints.getProduct}?employee_id=$employeeId$searchProduct$pageQuery$perPageQuery', method: HttpMethodType.GET)));
  ...
  static Future<ProductDetailResponse> getProductDetails({required int productId}) async {
    return ProductDetailResponse.fromJson(
        await handleResponse(await buildHttpResponse("${APIEndPoints.getProductDetails}?id=$productId&user_id=${loginUserData.value.id}", method: HttpMethodType.GET)));
  }
}
